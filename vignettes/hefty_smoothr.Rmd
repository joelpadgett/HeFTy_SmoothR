...
plot1 <- plot_path_density(tT_paths)
plot1 + guides(fill = 'none')
```
...
- `GOF_threshold` - A low number when only the best constrained paths should be considered, and a large number when even less constrained paths can be considered. 
- `densify` - Should extra points be added along the individual paths to ensure that not only the knickpoints of the path are evaluated? 
- `n` - How many equally-spaced extra points should be added along between the knickpoints of the path (if `densify=TRUE`).
...
plot2 <- plot_path_density(tT_paths, bins = 25, GOF_threshold = 5, densify = TRUE, n = 100, samples = 100) 
plot2 + guides(fill = 'none')
```

Finally, you can also customize the ggplot for labeling, change colors, and reverse the axes: 
